...
#include <mavsdk/plugins/command_robotic_vehicle/command_robotic_vehicle.h>
...
int main(int, char**)
{
    ...
    auto command_rv = CommandRoboticVehicle{target_system};

    command_rv.subscribe_command([&](CommandBase::CommandLong cmd) {
        std::cout << TELEMETRY_CONSOLE_TEXT // set to blue
                  << "CommandLong received: \n"
                  << cmd << NORMAL_CONSOLE_TEXT << std::endl;

        uint8_t result = MAV_RESULT_UNSUPPORTED;
        if(cmd.command == MAV_CMD_DO_SET_HOME_QUATERNION)
