...
int main(int, char**)
{
    ...
    auto telemetry = Telemetry{target_system};

    telemetry.subscribe_position_velocity_ned([&](TelemetryBase::PositionVelocityNed data) {
        std::cout << RESULT_CONSOLE_TEXT // set to blue
                  << "PositionVelocityNed received: \n"
                  << data << NORMAL_CONSOLE_TEXT << std::endl;
    });

    telemetry.subscribe_attitude([&](TelemetryBase::Attitude data) {
        std::cout << RESULT_CONSOLE_TEXT // set to blue
                  << "Attitude received: \n"
                  << data << NORMAL_CONSOLE_TEXT << std::endl;
    });

    telemetry.subscribe_text_status([&](TelemetryBase::TextStatus data) {
        std::cout << RESULT_CONSOLE_TEXT // set to blue
                  << "TextStatus received: \n"
                  << data << NORMAL_CONSOLE_TEXT << std::endl;
    });
